...
const HomePage = () => {
  ...
  const [customerPerMonth, setCustomerPerMonth] = useState(0);
  const [priceForProduct, setPriceForProduct] = useState(0);
  const [stateProcess, setStateProcess] = useState("Connecter vous avant de remplir le formulaire!");
 ...
 /* const calculatePrice = () => {
  ...
  const sendDataToSheet = async () => {
    ...
    try {
      ...
      if(result)
      {
        setStateProcess("Vous pouvez allez voir votre boîte mail!")
      }
      console.log("Success:", result);
    } catch (error) {
      setStateProcess("Une erreur c'est produite, si cela continue, contactez nous par mail: agence.aykin@gmail.com")
  ...
  const login = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      setStateProcess("Remplissez le formulaire puis valider le formulaire")
  ...
  return (
    <div className="bg-gray-100 text-gray-800 font-sans">
      <header className="bg-white shadow-md">
        <nav className="container mx-auto flex justify-between items-center py-4 px-6">
          ...
          <ul className="flex space-x-6">
            <li>
              <a
                href="#reliability-score"
                className=" text-xs text-gray-800 hover:text-primary"
              >
                Score de Fiabilité
              </a>
            </li>
      ...
      <div className="container mx-auto p-6">
        <section className="hero flex flex-col lg:flex-row items-center py-12">
          <div className="hero-text flex-1">
            <h1 className="text-5xl font-bold mb-4">
              Estimez la Fiabilité de Votre Projet
            </h1>
            <p className="text-xl mb-6">
              Obtenez des informations précieuses sur le potentiel de succès de
              votre projet avec notre outil avancé d'estimation de fiabilité.
            </p>
            {!token && (
              <button onClick={() => login()} className="btn btn-primary">
                Commencez l'Estimation en se connectant
              </button>
            )}
          </div>
          <div className="hero-image flex-1 text-right">
            <img
              src="./images/flat-representation.svg"
              alt="Représentation de l'outil"
              className="max-w-full h-auto"
            />
          </div>
        </section>

        <section className="feature-card bg-white rounded-lg p-6 shadow-md">
          <h3 className="text-lg font-bold mb-4 text-center">
            Quelques informations sur vous
          </h3>
          <p>
            Après avoir remplis tous les champs du formulaire, vous recevrez les
            résultats par mail. Renseignez donc correctement le champ email
          </p>
          <p>
            {" "}
            Vos informations serons conservés en base de donnée et nous
            pourrions vous envoyer des emails pour vous propoer nos offres et
            actualités.
          </p>
          <div>
            <label className="block mb-2 ">Nom Prénom</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block mb-2 ">Quel est votre produit?</label>
            <input
              type="text"
              value={product}
          ...
          <div>
            <label className="block mb-2 ">
              Combien de clients souhaité vous avoir par mois?
            </label>
            <input
              type="number"
          ...
          <div>
            <label className="block mb-2 ">
              A quel prix estimé vous votre produit?
            </label>
            <input
              type="number"
        ...
        <section
          id="features"
          className="features grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-12"
        >
          <div className="feature-card bg-white rounded-lg p-6 shadow-md">
            ...
            <div>
              <label className="block mb-2 ">
                Nombre de jours travaillé dans la boîte dans un an (JT)
              </label>
              <input
                type="number"
                value={JT}
            ...
            <div>
              <label className="block mb-2 ">
                Salaire mensuel d’un dirigeant (S)
              </label>
              <input
                type="number"
                value={S}
            ...
            <div>
              <label className="block mb-2 ">
                Pourcentage représentant le nombre de jours travaillés en tenant
                en compte les aléas de la vie (P_alea_vie)
              </label>
              <input
                type="number"
                value={P_alea_vie}
            ...
            <div>
              <label className="block mb-2 ">
                Pourcentage représentant les arrêts maladies liés aux salariés
                (P_arret)
              </label>
              <input
                type="number"
            ...
            <div>
              <label className="block mb-2 ">
                Pourcentage passé sur un produit 1 développé par la boîte
                (P_produit)
              </label>
              <input
                type="number"
          ...
          <div className="feature-card bg-white rounded-lg p-6 shadow-md">
            ...
            <div>
              <label className="block mb-2 ">
                Coût direct par mois (DIRECT_COST)
              </label>
              <input
                type="number"
                value={DIRECT_COST}
            ...
            <div>
              <label className="block mb-2 ">
                Somme des salaires brut annuels des salariés à l’exception du
                dirigeant ayant travaillé sur le produit 1 (MSP1)
              </label>
              <input
                type="number"
                value={MSP1}
            ...
            <div>
              <label className="block mb-2 ">
                Somme des salaires brut annuels des salariés à l’exception du
                dirigeant ne travaillant pas sur le produit 1 (MSP2)
              </label>
              <input
                type="number"
                value={MSP2}
          ...
          <div className="feature-card bg-white rounded-lg p-6 shadow-md">
            ...
            <div>
              <label className="block mb-2 ">
                Pourcentage passé sur le produit 1 (P)
              </label>
              <input
                type="number"
                value={P}
            ...
            <div>
              <label className="block mb-2 ">
                Nombre de salariés ayant travaillé sur le produit 1 (NSP1)
              </label>
              <input
                type="number"
                value={NSP1}
            ...
            <div>
              <label className="block mb-2 ">
                Nombre de jours travaillé par le dirigeant (NJ)
              </label>
              <input
                type="number"
                value={NJ}
        ...
        <section
          ...
          className="reliability-score bg-neutral text-neutral-content text-center py-12 mt-12 rounded-lg"
        >
          <h2 className="text-4xl font-bold mb-4">
            Score de Fiabilité de Votre Projet
          </h2>
